...
// используется делитель частоты на 3 
// ============================================================================
module parallel_master (
    ...
    input  logic        ext_ready_i,    // apb готов к обмену
    input  logic        ext_ack_i,      // Подтверждение транзакции/слова
    input  logic [15:0] ext_rdata_i,    // Данные от apb (16 бит)
    ...
    typedef enum logic [2:0] {
        IDLE,         // Ожидание запроса от FSM (req_i)
        ADDR_PHASE,   // Выставление адреса на шину, ожидание ext_ready_i
        DATA_LOW,     // Приём/передача младшей половины данных (16 бит)
        DATA_HIGH,    // Приём/передача старшей половины данных
        WAIT_ACK      // Ожидание подтверждения ext_ack_i от ведомого,
                      // затем переход к следующему слову или завершение
    } state_t;
    ...
    // Внешний интерфейс работает в 3 раза медленнее внутреннего.
    // ========================================================================
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                // ------------------------------------------------------------
                // IDLE – ждём запроса от FSM
                // ------------------------------------------------------------
                IDLE: begin
                    ext_cmd_o  <= 2'b00;   // Нет активности на шине
                    ext_data_o <= 16'b0;
                    if (req_i) begin
                        stream_mode <= wr_i[1];   // Запомнить режим (поток/одиночный)
                        word_cnt    <= 2'b0;
                        state       <= ADDR_PHASE;
                    end
                end
                ...
                // ------------------------------------------------------------
                // ADDR_PHASE – выставляем адрес, ждём готовности ведомого
                // ------------------------------------------------------------
                ADDR_PHASE: begin
                    ...
                    if (ext_ready_i) begin         // Ведомый готов
                        if (wr_i[0]) begin         // Операция записи
                            ...
                            ext_data_o <= wdata_i[15:0];  // Младшая половина на шину
                ...
                // ------------------------------------------------------------
                // DATA_LOW – приём младшей половины (только для чтения)
                // ------------------------------------------------------------
                DATA_LOW: begin
                    ext_addr_o <= addr_i;
                    ext_cmd_o  <= 2'b10;                // Код «чтение»
                ...
                // ------------------------------------------------------------
                // DATA_HIGH – передача (запись) или приём (чтение) старшей половины
                // ------------------------------------------------------------
                DATA_HIGH: begin
                    ext_addr_o <= addr_i;
                    if (wr_i[0]) begin                   // Запись
                        ext_data_o <= wdata_i[31:16];    // Старшая половина на шину
                        ext_cmd_o  <= 2'b11;             // Код «запись»
                        state <= WAIT_ACK;               // Ждать подтверждения слова
                    end else begin                       // Чтение
                        ext_cmd_o  <= 2'b10;             // Код «чтение»
                        if (ext_ready_i) begin
                            // Собираем 32-битное слово из младшей (data_low) и старшей частей
                ...
                // ------------------------------------------------------------
                // WAIT_ACK – ожидание подтверждения ext_ack_i от ведомого,
                // после чего либо переходим к следующему слову потока, либо завершаем
                // ------------------------------------------------------------
                WAIT_ACK: begin
                    ext_addr_o  <= addr_i;
                    ...
                    if (ext_ack_i) begin                     // Ведомый подтвердил
                        ...
                        if (stream_mode && (word_cnt < 2'd3)) begin // Есть ещё слова в потоке
                            ...
                            state    <= ADDR_PHASE;          // Следующее слово (адрес увеличивается снаружи)
                        end else begin
                            word_cnt <= 2'b0;
                            done_o   <= 1'b1;                // Транзакция полностью завершена
                            state    <= IDLE;
                        end
                    end
                end
                ...
                // ------------------------------------------------------------
                // Защита от неопределённого состояния
                // ------------------------------------------------------------
                default: begin
                    state      <= IDLE;
                    ext_cmd_o  <= 2'b00;
