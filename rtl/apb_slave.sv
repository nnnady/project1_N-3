...
// master, параллельный внешний интерфейс, геометрическая прогрессия, ОЗУ.
// ============================================================================
module apb_slave (
    ...
    input  logic [31:0] pwdata,       // Данные для записи, шина данных
    ...
    output logic        data_written_o, // сигнал, что данные записаны в регистр данных (адрес 0x10), для потоковых операций
    input  logic [31:0] fsm_wdata_i,  // Данные, пришедшие от FSM (для чтения по 0x10)
    output logic        apb_rdata_pop // сигнал, что APB прочитал данные из регистра данных (нужен FSM)
);
    // ========================================================================
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            //используется неблокирующее присваивание, чтобы записать в регистры в конце текущего такта
            ADDR_REG   <= 16'b0; 
            DATA_REG   <= 32'b0;
            CMD_REG    <= 2'b0;
            STATUS_REG <= 1'b0;
            CTRL_REG   <= 1'b0;
        end else if (psel && penable && pwrite && !pslverr) begin // мастер выбрал это ус-во, транзакция перешла в фазу данных, это операция записи,  ошибок нет
        ...
        // Регистр статуса аппаратно обновляется сигналом busy от FSM (чтобы понимать свободен ли конвертор)
        STATUS_REG <= busy_i;
    end

    // ========================================================================
    // Чтение регистров (комбинационная логика, нет задержки на такт)
    // ========================================================================
    always_comb begin
        if (!pwrite) begin
            case (paddr[15:0])
                ...
                16'h0010: prdata = fsm_wdata_i; // для чтения используем отдельный канал от fsm          // Данные, предоставленные FSM
    ...
    // ========================================================================
    always_comb begin
        if (psel && penable) begin //ус-во выбрано и разрешена подача
            if ((pwrite && paddr == 16'h0000) || //регистр статусы только для чтения, если мастер пытается туда писать то это ошибка
                (!pwrite && paddr == 16'h0004) || //попытка чтения из регистра управления (он только для записи)
    ...
    //формирование сигнала что apb готов завершить транзакцию
    assign pready = psel & penable;

    // сигнал записи данных (для FSM, чтобы сохранить данные в ОЗУ при потоковой записи)
